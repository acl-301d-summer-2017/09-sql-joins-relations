...
const app = express();
const conString = 'postgres://postgres:newguy@localhost:5432/kilovolt';
...
app.get('/articles', function(request, response) {
  // TODO DONE: Write a SQL query which joins all data from articles and authors tables on the 
  //author_id value of each
  client.query('SELECT * FROM articles JOIN authors ON article.author_id=authors.author_id;')
...
app.post('/articles', function(request, response) {
  // TODO DONE: Write a SQL query to insert a new author into the authors table, ON CONFLICT DO NOTHING
  ...
  function queryTwo() {
    client.query(
      `SELECT author_id FROM authors WHERE author=$1`, // TODO DONE: Write a SQL query to retrieve the author_id from the authors table for the 
      // new article
      [request.author.name], // TODO DONE: Add the author name as data for the SQL query
      function(err, result) {
        if (err) console.error(err)
        queryThree(result.rows[0].author_id) // This is our third query, to be executed when the 
        // second is complete. We are also passing the author_id into our third query
      }
    )
  }
  ...
  function queryThree(author_id) {
    ...
    client.query(
      ...
      [author_id, response.body.title, response.body.category, response.body.publishedOn, response.body.body], 
      function(err) {
        if (err) console.error(err);
...
app.put('/articles/:id', function(request, response) {
  ...
  client.query(
    `UPDATE authors SET author=$1, authorUrl=$3 WHERE author_id=$2;`,
    [request.body.author, request.body.author_id, request.body.authorUrl]
  )
  .then(function() {
